...
If the function you are posthooking has a returning code, that value is available on the posthook function as `${1}`. The return done on the posthook function will be the final return value for the function overriding the original one. So **if you are posthooking a function with return codes you must have a mandatory return statement on the posthook function**.
