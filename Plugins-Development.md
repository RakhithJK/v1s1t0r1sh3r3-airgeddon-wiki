...
If the function you are posthooking has a returning code, that value is available on the posthook function as `${1}`. The return done on the posthook function will be the final return value for the function overriding the original one. If there is no return value on the posthook function, then the return value will be the original one.
